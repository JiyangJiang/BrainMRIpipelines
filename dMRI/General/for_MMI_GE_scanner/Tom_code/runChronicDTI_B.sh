...
echo
echo "*****************************************************************************"
echo "***  Multishell blipped diffusion blipped 3 dataset MRI pipeline          ***"
...
THIS INCLUDES 3 LARGE DIFFUSION DATASETS (AND BLIPPING)
...
##==============================================================================
if [ -z ${nii1+x} ]; then
    echo " ERROR :  nii1 file is unset "
    usage
    exit 1
fi
if [ -z ${bvec1+x} ]; then
    echo " ERROR :  bvec1 file is unset "
    usage
    exit 1
fi
if [ -z ${bval1+x} ]; then
    echo " ERROR :  bval1 file is unset "
    usage
    exit 1
fi
if [ -z ${nii2+x} ]; then
    echo " ERROR :  nii2 file is unset "
    usage
    exit 1
fi
if [ -z ${bvec2+x} ]; then
    echo " ERROR :  bvec2 file is unset "
    usage
    exit 1
fi
if [ -z ${bval2+x} ]; then
    echo " ERROR :  bval2 file is unset "
    usage
    exit 1
fi
if [ -z ${nii3+x} ]; then
    echo " ERROR :  nii3 file is unset "
    usage
    exit 1
fi
if [ -z ${bvec3+x} ]; then
    echo " ERROR :  bvec3 file is unset "
    usage
    exit 1
fi
if [ -z ${bval3+x} ]; then
    echo " ERROR :  bval3 file is unset "
    usage
    exit 1
fi
if [ -z ${niiFLIP+x} ]; then
    echo " ERROR :  flipped blipped nii file is unset "
    usage
    exit 1
fi
if [ -z ${SUFFIX+x} ]; then
...
echo "START: " $(date) 
for (( i=1; i<4; i++ )); do
    blipCorrectionTopup_REVISED.sh -f $niiFLIP -o ${outDIRL[$i-1]} -s $SUFFIX &
done
wait
echo "END: " $(date) " ===============////====================="
...
echo "START: " $(date) 
for (( i=1; i<4; i++ )); do
    eddyRunBlipped_REVISED.sh -o ${outDIRL[$i-1]} -s $SUFFIX
done
echo "END: " $(date) " ===============////====================="
...
echo " ######## PROCESSING COMPLETE FOR DENOISING, TOPUP, EDDY, MRTRIX3 TRACTOGRAPHY PIPELINE ######## "
