...
outdir = '/data_int/jiyang/FLICA/no_dementia/output_FreeSurferThickAreaSmooth15_DMN_FPCN_d100_5000perm/';
...
['/data_int/jiyang/FLICA/no_dementia/DMN_Zmap_N310.nii.gz'],
['/data_int/jiyang/FLICA/no_dementia/FPCN_Zmap_N310.nii.gz']
};

transformsIn = {'','','',''; 'log','','',''; '','','','';'','','',''};

[Y,fileinfo] = flica_load(Yfiles, transformsIn);
fileinfo.shortNames = {'thickness','area','DMNactivationMap','FPCNactivationMap'};
...
#
cd /data_int/jiyang/FLICA/no_dementia/output_FreeSurferThickArea_DMN_FPCN_d100_5000perm
...
#
cd /data_int/jiyang/FLICA/no_dementia/output_FreeSurferThickArea_DMN_FPCN_d100_5000perm
fslmaths niftiOut_mi1.nii.gz -sqr tmp1.nii.gz;fslstats -t tmp1.nii.gz -m > energy1.txt;fslmaths niftiOut_mi2.nii.gz -sqr tmp2.nii.gz;fslstats -t tmp2.nii.gz -m > energy2.txt;fslmaths niftiOut_mi3.nii.gz -sqr tmp3.nii.gz;fslstats -t tmp3.nii.gz -m > energy3.txt;fslmaths niftiOut_mi4.nii.gz -sqr tmp4.nii.gz;fslstats -t tmp4.nii.gz -m > energy4.txt;paste energy1.txt energy2.txt energy3.txt energy4.txt > energy_tmp.txt
...
for i = 1:size(energy,1)
	fprintf(fid_energy,'%.5f\t%.5f\t%.5f\t%.5f\n',energy(i,1),energy(i,2),energy(i,3),energy(i,4));
end

% save environment/variables in MATLAB
cd /data_int/jiyang/FLICA/no_dementia
save ('output_FreeSurferThickArea_DMN_FPCN_d100_5000perm', '-v7.3');