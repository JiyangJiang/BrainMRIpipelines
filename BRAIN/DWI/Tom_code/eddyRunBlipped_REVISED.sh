...
{
cat << EOF
EDDY CURRENT CORRECTION FOR BLIP-CORRECTED-AND-DENOISED DWI DATA
Inputs:    Suffix and Working directory containing:
                acqparams.txt, topup_b0_blips*, dwi[suffix].nii
...
echo "" > "$INPUTWORKING"/acqparams_$vols.txt
...
# NEED TO CREATE A BVAL FILE WITH THE CORRECT BVALUES.
# THIS IS DONE BY CONVERTING THE NII, BVAL AND BVEC FROM DCM2NII INTO MIF FORMAT
# AND THEN EXTRACTING THE CORRECT BVAL (AND BVEC) FROM THAT MIF FILE

##-ADDED BVECS FILE FROM TENSOR9.DAT-------------------
scriptDir=$(dirname $(which eddyRunBlipped_REVISED.sh))
case $vols in
	68)
...
mrconvert -force \
          ...
          -fslgrad $bvecs "$INPUTWORKING"/bval"$INPUTSUFFIX"*.* \
          "$INPUTWORKING"/dwinii2mifpreeddy.mif

mrinfo -force \
       "$INPUTWORKING"/dwinii2mifpreeddy.mif \
       -export_grad_fsl "$INPUTWORKING"/mrtrixbvec.txt "$INPUTWORKING"/mrtrixbval.txt

##==============================================================================
echo " ======= RUNNING EDDY. NOTE THAT THIS CAN TAKE A LONG TIME ======= "
echo
eddy_openmp --imain="$INPUTWORKING"/dwidenoised"$INPUTSUFFIX" \
            --mask="$INPUTWORKING"/nodif_brain_mask \
            --index="$INPUTWORKING"/index$vols.txt \
            --acqp="$INPUTWORKING"/acqparams_$vols.txt \
            --bvecs="$INPUTWORKING"/mrtrixbvec.txt \
            --bvals="$INPUTWORKING"/mrtrixbval.txt \
            --fwhm=0 \
            --topup="$INPUTWORKING"/topup_b0_blips \
            --flm=quadratic \
            --out="$INPUTWORKING"/eddy_unwarped_images"$INPUTSUFFIX" \
            --data_is_shelled \
            --verbose
echo
...
mv "$INPUTWORKING"/eddy_unwarped_images"$INPUTSUFFIX".eddy_rotated_bvecs \
   "$INPUTWORKING"/rotatedbvec.txt
