...
% Last Modified by GUIDE v2.5 19-Jun-2016 11:44:08
...
if get(handles.DICOMtoNIFTI_checkbox,'Value') == 1
    ...
    fprintf('Converting Functional Images Starts.\n');
    parfor i = 1:SubNum
    ...
    if ~isempty(dir([WorkDir,filesep,'T1Raw'])) && isempty(dir([WorkDir,filesep,'T1Img']))
        ...
        parfor i = 1:SubNum
            ...
            mkdir(OutputDir);
            fprintf(['Converting T1 Images:',Sublist{i},' Starts.\n']);
            dcm2nii(Filename, OutputDir);
            fprintf(['Converting T1 Images:',Sublist{i},' OK.\n']);
        end
    end
    fprintf('Converting T1 Images Finishes.\n');
    cd(WorkDir);
end
...
if get(handles.RemoveTPS_checkbox,'Value') == 1
    ...
    fprintf(['Removing First ',get(handles.RemoveTPS_edit,'String'),' Images Starts.\n']);
    parfor i=1:SubNum
        fprintf(['Removing First ',num2str(DeleteNum),' Time Points: ',Sublist{i},' Starts.\n']);
    ...
    fprintf(['Removing First ',get(handles.RemoveTPS_edit,'String'),' Images Finishes.\n']);
    cd(WorkDir);
end

% Slice timing
if get(handles.SliceTiming_checkbox,'Value') == 1
    fprintf('Slice Timing Starts.\n');
    ScanOrder = get(handles.SliceTiming_popupmenu,'Value');
    TR = str2double(get(handles.TR_edit,'String'));
    parfor i=1:SubNum
        fprintf(['Slice Timing:',Sublist{i},' Starts.\n']);
        ...
        switch ScanOrder
            case 1
                if nSlice/2 == floor(nSlice/2)
                    spmjob.matlabbatch{1,1}.spm.temporal.st.so = [2:2:nSlice,1:2:nSlice - 1];
                else
                    spmjob.matlabbatch{1,1}.spm.temporal.st.so = [1:2:nSlice,2:2:nSlice - 1];
                end
            case 2
                if nSlice/2 == floor(nSlice/2)
...
if get(handles.Normalize_checkbox,'Value') == 1
    ...
    switch get(handles.Normalize_popupmenu,'Value')
        case 1
            % Using EPI template
            parfor i = 1:SubNum
                fprintf(['Normalize:',Sublist{i},' Starts.\n']);
                cd([WorkDir,filesep,StartFolder,filesep,Sublist{i}]);
                Filename = dir('*.nii');
                Nii = nifti(Filename.name);
                FileList = cell(Nii.dat.dim(1,4),1);
        ...
        case 2
            ...
            parfor i = 1:SubNum
                ...
                fprintf(['Coregester T1 to Fun:',Sublist{i},' Starts.\n']);
                ...
                % Segment
                fprintf(['Segment:',Sublist{i},' Starts.\n']);
                ...
                fprintf(['Normalize-Write:',Sublist{i},' Starts.\n']);
    ...
    fprintf('Normalization Finishes.\n');
    cd(WorkDir);
end
...
% Smooth
if get(handles.Smooth_checkbox,'Value') == 1
    fprintf('Smoothing Starts.\n');
    FWHM = eval(['[',get(handles.Smooth_edit,'String'),']']);
    parfor i = 1:SubNum
        fprintf(['Smoothing:',Sublist{i},' Starts.\n']);
    ...
    cd(WorkDir);
    fprintf('Smoothing Finishes.\n');
end

% Detrend
if get(handles.Detrend_checkbox,'Value') == 1
    fprintf('Detrend Starts.\n');
    parfor i = 1:SubNum
        fprintf(['Detrend:',Sublist{i},' Starts.\n']);
        cd([WorkDir,filesep,StartFolder,filesep,Sublist{i}])
        Filename = dir('*.nii');
        Nii = nifti(Filename.name);
    ...
    cd(WorkDir);
    fprintf('Detrend Finishes.\n');
end

% Regress Covariates
if get(handles.Regress_checkbox,'Value') == 1
    fprintf('Regressing Out Covariates Starts.\n');
    ...
    parfor i = 1:SubNum
        fprintf(['Regressing Out Covariates:',Sublist{i},' Starts.\n']);
        HMCov = [];
        WMCov = [];
        CSFCov = [];
        GSCov = [];
