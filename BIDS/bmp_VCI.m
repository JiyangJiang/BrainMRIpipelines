...
function orgniseDICOMdir (dcm_coll, DICOM_parent_folder, modality, str_toBeReplaced, str_replaceTo)
	...
	switch modality
		...
		case 'SWI'
			...
			series_description_keyword = 'greME7_p31_256_Iso1mm';

		case 'DTI'

			series_description_keyword = 'DIFFUSION';

		case 'ASL'

			series_description_keyword = 'asl_3d_tra_p2_iso_3mm_highres';

		case 'CVR_fmap'

			series_description_keyword = 'gre_field_mapping 3.8mm';

		case 'CVR'

			series_description_keyword = 'CVR_ep2d_bold 3.8mm TR1500 adaptive';

		case 'MP2RAGE'

			series_description_keyword = 't1_mp2rage_sag_0.8x0.8x2';

		case 'DCE'

			series_description_keyword = 't1_vibe_sag_DCE_2mm XL FOV 40s temporal res';
	...
	for i = 1 : size(curr_dcmCollection_SeriesDescUniqueVal,1)
		...
		% fprintf ('%s : Note that since %s is 4D image (i.e., for each unique SeriesDescription, there''re multiple 3D images), we use SeriesInstanceUID as sub-folder names to separate these 3D images.\n', mfilename, modality);
