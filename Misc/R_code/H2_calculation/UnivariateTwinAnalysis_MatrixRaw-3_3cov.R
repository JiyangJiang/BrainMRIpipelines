...
# ASL_results = read.csv("FSL_shrinked_wide_forR.csv", header = TRUE)
...
# FSL #
# === #
# selVars = c("globalCBF_native_inclCSF_1", "globalCBF_native_inclCSF_2")
# selVars = c("gmCBF_native_pvcorr_FSL_1", "gmCBF_native_pvcorr_FSL_2")
# selVars = c("wmCBF_native_pvcorr_FSL_1", "wmCBF_native_pvcorr_FSL_2")
# selVars = c("globalCBF_native_exclCSF_1", "globalCBF_native_exclCSF_2")
# selVars = c("gmCBF_native_pvcorr_Jmod_1", "gmCBF_native_pvcorr_Jmod_2")
# selVars = c("wmCBF_native_pvcorr_Jmod_1", "wmCBF_native_pvcorr_Jmod_2")
# selVars = c("caudate_1", "caudate_2")
# selVars = c("cerebellum_1", "cerebellum_2")
# selVars = c("frontal_lobe_1", "frontal_lobe_2")
# selVars = c("insula_1", "insula_2")
# selVars = c("occipital_lobe_1", "occipital_lobe_2")
# selVars = c("parietal_lobe_1", "parietal_lobe_2")
# selVars = c("putamen_1", "putamen_2")
# selVars = c("temporal_lobe_1", "temporal_lobe_2")
# selVars = c("thalamus_1", "thalamus_2")
...
# 0/1 for pCASL/PASL
ASLtypeT1MZ <- as.matrix(subset(ASL_results, zyg=="MZ", pCASL_vs_PASL_1))
ASLtypeT2MZ <- as.matrix(subset(ASL_results, zyg=="MZ", pCASL_vs_PASL_2))
ASLtypeT1DZ <- as.matrix(subset(ASL_results, zyg=="DZ", pCASL_vs_PASL_1))
ASLtypeT2DZ <- as.matrix(subset(ASL_results, zyg=="DZ", pCASL_vs_PASL_2))

mzData <- data.frame(mzfData,ageT1MZ,ageT2MZ,sexT1MZ,sexT2MZ,ASLtypeT1MZ,ASLtypeT2MZ)
dzData <- data.frame(dzfData,ageT1DZ,ageT2DZ,sexT1DZ,sexT2DZ,ASLtypeT1DZ,ASLtypeT2DZ)
...
twinACEModel <- mxModel("twinACE",
    ...
    mxMatrix( type="Full", nrow=1, ncol=Ncov, free=TRUE, values= 0, label=c("betaAge","betaSex","betaASLtype"), name="beta"),
	...
	mxModel("MZ", mxData( observed=mzData, type="raw" ),
	        mxMatrix( type="Full", nrow=Ncov, ncol=2, free=F, label=c("data.age_in_day_1",
	        													   "data.Sex01_1",
	        													   "data.pCASL_vs_PASL_1",
	        													   "data.age_in_day_2",
	        													   "data.Sex01_2",
	        													   "data.pCASL_vs_PASL_2"), 
	        												name="MZDefVars"),
